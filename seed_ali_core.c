...
int seed_ali()
{
	...
	chr_res = (uint8_t** )calloc(thread_n, sizeof(uint8_t* ));
	for(r_i = 0; r_i < thread_n; r_i++)
		chr_res[r_i] = (uint8_t* )calloc(CUS_MAX_OUTPUT_ALI2, 1);
	...
	chr_res_buffer = (uint8_t** )calloc(read_in, sizeof(uint8_t* ));
	for(r_i = 0; r_i < read_in; r_i++)
		chr_res_buffer[r_i] = (uint8_t* )calloc(CUS_MAX_OUTPUT_ALI2, 1);
	...
	for(r_i = 1; r_i < chr_file_n; r_i++)
		fprintf(fp_sam, "@SQ SN:%s\tLN:%u\n", chr_names[r_i], chr_end_n[r_i] - chr_end_n[r_i - 1]);
	...
	while ((kr1 > 0) && (kr2 > 0))
    {
		...
		for(seqi = 0; seqi < seqii; seqi++)
		{
			...
			{
				fprintf(fp_sam, "%s\t%u\t%s\t%"PRId64"\t%d\t%s\t=\t%"PRId64"\t%"PRId64"\t%s\t%s",
					seqio[seqi].name, seqio[seqi].flag1, chr_names[seqio[seqi].chr_re], seqio[seqi].pos1,
					seqio[seqi].qualc1, seqio[seqi].cigar1, seqio[seqi].pos2, seqio[seqi].cross, seqio[seqi].seq1,
					seqio[seqi].qual1
				);
				...
				{
					if((seqio[seqi].xa_n == 0) && (seqio[seqi].xa_n1 > 0))	fprintf(fp_sam, "\tXA:Z:");
				...
				fprintf(fp_sam, "%s\t%u\t%s\t%"PRId64"\t%d\t%s%s\t=\t%"PRId64"\t%"PRId64"\t%s\t%s",
					seqio[seqi].name, seqio[seqi].flag1, chr_names[seqio[seqi].chr_re], seqio[seqi].pos1,
					seqio[seqi].qualc1, seqio[seqi].cigar1, h_chars, seqio[seqi].pos2, seqio[seqi].cross, seqio[seqi].seq1,
					seqio[seqi].qual1
				);
			...
			{
				fprintf(fp_sam, "%s\t%u\t%s\t%"PRId64"\t%d\t%s\t=\t%"PRId64"\t%"PRId64"\t%s\t%s",
						seqio[seqi].name_other, seqio[seqi].flag2, chr_names[seqio[seqi].chr_re], seqio[seqi].pos2,
						seqio[seqi].qualc2, seqio[seqi].cigar2, seqio[seqi].pos1, -seqio[seqi].cross, seqio[seqi].seq2,
						seqio[seqi].qual2
				);
				...
				{
					if((seqio[seqi].xa_n == 0) && (seqio[seqi].xa_n2 > 0))	fprintf(fp_sam, "\tXA:Z:");
				...
				fprintf(fp_sam, "%s\t%u\t%s\t%"PRId64"\t%d\t%s%s\t=\t%"PRId64"\t%"PRId64"\t%s\t%s",
						seqio[seqi].name_other, seqio[seqi].flag2, chr_names[seqio[seqi].chr_re], seqio[seqi].pos2,
						seqio[seqi].qualc2, seqio[seqi].cigar2, h_chars, seqio[seqi].pos1, -seqio[seqi].cross, seqio[seqi].seq2,
						seqio[seqi].qual2
				);
    ...
    for(seqi = 0; seqi < seqn; seqi++)
    {
                    ...
                    if((end1_uc_f == 0) && ((dmt1 <= d_l1) && (dmt1 <= d_r1)))// && (nuc1_f == 0)
                    {
                        if(nuc1_f == 0)
                        {
                            ...
                            if(re_d != 0)
                            {
                                ...
                                ref_seq_tmp1[tid][rst_i] &= low_mask1[tid];
                            }
                            else
                            {
                                ...
                                //clear the lowest n bit
                                ref_seq_tmp1[tid][ref_copy_num_1 - 1] &= low_mask1[tid];
                            }
                            ...
                            ref_seq_tmp1[tid][ref_copy_num_1 - 2] &= low_mask1[tid];
                    ...
                    if((end2_uc_f == 0) && ((dmt2 <= d_l2) && (dmt2 <= d_r2)))// && (nuc2_f == 0)
                    {
                        if(nuc2_f == 0)
                        {
                            ...
                            if(re_d != 0)
                            {
                                ...
                                //clear the lowest n bit
                                ref_seq_tmp2[tid][rst_i] &= low_mask2[tid];
                            ...
                            else
                            {
                                ...
                                //clear the lowest n bit
                                ref_seq_tmp2[tid][ref_copy_num_2 - 1] &= low_mask2[tid];
                            }
                            ...
                            ref_seq_tmp2[tid][ref_copy_num_2 - 2] &= low_mask2[tid];
        ...
        if(de_m_p_o[tid] == 1)
        {
            ...
            if(v_cnt > 0)
            {
				...
				else  //indel
				{
					...
					else if((lv_up_right < lv_up_left) && (lv_down_right > lv_down_left))//op_dm_l1[tid][v_cnt_i] == -1
					{
						if(b_cigar[pchl] == 'M')
						{
							...
							sn = sprintf(cigar_p1, "%uM%s", m_m_n, b_cigar);
							snt += sn;
						}
					}
				...
				}else if((sam_flag1 == 83) || (sam_flag1 == 89))
				{
            ...
            for(va_cnt_i = 1; va_cnt_i < v_cnt; va_cnt_i++)
            {
                ...
                sam_pos1s[tid][xa_i] = (uint32_t )sam_pos1;
                sam_pos2s[tid][xa_i] = (uint32_t )sam_pos2;

				if((sam_flag1 == 117) || (sam_flag1 == 69))
				{
					strcpy(cigar_p1s[tid][xa_i], "*");
					strcpy(cigar_p2s[tid][xa_i], cp2);
				}
				else if((sam_flag1 == 73) || (sam_flag1 == 89))
				{
					strcpy(cigar_p1s[tid][xa_i], cp1);
                ...
                lv_re1s[tid][xa_i] = lv_re1b;
                lv_re2s[tid][xa_i] = lv_re2b;
                ++xa_i;
            ...
            if(v_cnt > 0)
            {
                ...
                if(xa_i > 0)
                {
					memcpy(chr_res_buffer[seqi], chr_res[tid], xa_i);
        ...
        else//indel
        {
			if((cir_n == cir_fix_n) && (local_ksw))
			{
                ...
                else if ((lv_up_right < lv_up_left) && (lv_down_right > lv_down_left))//op_dm_l1[tid][v_cnt_i] == -1
                {
                    ...
                    {
                        sn = sprintf(cigar_p2, "%uM%s", m_m_n, b_cigar);
                        snt += sn;
                    }
                }
                else if ((lv_down_right <= lv_down_left) && (lv_up_right >= lv_up_left))
					...
					if((s_r_o_l != -1) && (s_r_o_r != read_length))
					{
						...
						for (bit_char_i = n_cigar1 - 1; bit_char_i > k_start1; bit_char_i--) {
							...
							if (op_score == 0) { // match
								...
								x_score += len_score; y_score += len_score;
							} else if (op_score == 1) x_score += len_score, nm_score += len_score, i_n1 += len_score;
							else if (op_score == 2) y_score += len_score, nm_score += len_score, d_n1 += len_score;
						}
